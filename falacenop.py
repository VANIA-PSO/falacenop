...
modo = st.sidebar.radio("Selecione a exibição:", ["Interface do Funcionário (Chat)", "Painel da Gestão (Análises)"], key="navegacao_principal")
...
if modo == "Interface do Funcionário (Chat)":
    ...
    if st.session_state.step == 1:
        ...
        with col1:
            if st.button("🟢 Motivado(a)\n\n(Fluxo normal)", use_container_width=True, key="btn_motivado"):
        ...
        motivo_selecionado = st.radio("Selecione o causador principal:", motivos, key="radio_motivos")
        comentario = st.text_area("Observação opcional para a gestão (privado):", placeholder="Escreva detalhes aqui se desejar...", key="txt_comentario")

        if st.button("Enviar Registro", type="primary", use_container_width=True, key="btn_enviar_registro"):
        ...
        # Seção de Sugestões com chave única (key)
        st.markdown("---")
        st.markdown("### 💡 Espaço de Contribuição")
        quer_sugerir = st.radio("Gostaria de deixar alguma sugestão?", ["Sim", "Não"], key="radio_sugestao")
        
        if quer_sugerir == "Sim":
            sugestao_texto = st.text_area("Digite sua sugestão aqui:", key="txt_sugestao")
            if st.button("Enviar Sugestão", key="btn_enviar_sugestao"):
        ...
        if st.button("🔄 Enviar outra resposta", type="secondary", key="btn_reiniciar"):
            st.session_state.step = 1
            st.session_state.resposta_farol = None
            st.session_state.mensagem_suporte = ""
            st.rerun()
